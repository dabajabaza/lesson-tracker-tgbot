...
- **Жёсткий бэкстоп — systemd watchdog** (`bot/watchdog.py`): бот пингует systemd
  (`WATCHDOG=1`) только пока реально достукивается до Telegram через прокси (проба
  `getMe` раз в 30с). Завис дольше `WatchdogSec=90` — systemd убивает процесс и
  поднимает заново (`Restart=always`). Вне systemd (запуск вручную) watchdog — no-op.
