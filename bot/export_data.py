...
STUDENT_HEADERS = [
    "ID", "Имя", "Стоимость занятия, руб", "Осталось занятий",
    "Денежный остаток, руб", "Последняя оплата", "Сумма последней оплаты, руб",
    "Занятий в последней оплате",
]
HISTORY_HEADERS = [
    "Дата и время", "ID ученика", "Имя", "Операция", "Сумма, руб",
    "Изменение занятий", "Занятий после", "Денежный остаток после, руб",
    "Новая стоимость, руб", "Отменено",
]


def students_rows(students) -> list[list]:
    rows = [STUDENT_HEADERS]
    for s in students:
        rows.append([
            s.id, s.name, to_rubles(s.price), s.balance, to_rubles(s.remainder),
