...
class Invite(Base):
    ...
    проверкой в коде, а UPDATE ... WHERE used_by IS NULL (см. services/access.py):
...
class OutboxMessage(Base):
    ...
    Порядок «обработчик → коммит → отправка» (см. bot/middlewares/) снял потолок
    ...
    отправить — строка осталась, и её дожмёт фоновый отправщик (runtime/outbox.py).
