"""Единица работы: транзакция на апдейт + сериализация пишущих апдейтов.
...
делает такую гонку невозможной внутри процесса; `BEGIN IMMEDIATE` (см. db/engine.py)
...
сеть сами, они складывают намерения в Responder (см. bot/ui.py), и сеть случается
...
class DbSessionMiddleware(BaseMiddleware):
    ...
    def __init__(self, lock: asyncio.Lock) -> None:
        ...
        # фоновому отправщику (runtime/outbox.py) — писатель в SQLite один на процесс,
    ...
    async def _run(self, handler, event, data, ui: Responder):
        ...
        if update_id is not None:
            ...
            if seen is not None:
                ...
                # транзакцию, и по умолчанию она у нас IMMEDIATE (db/engine.py), то есть
