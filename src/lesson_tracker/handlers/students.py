...
async def on_new_name(
    ...
    # prompt_id обнуляется вместе с постановкой удаления в очередь: со
    # следующей строки старая подсказка считается мёртвой, и оставлять на неё
    # указатель нельзя. Пока он оставался, быстрый следующий апдейт (пришедший
    # раньше, чем _settle допишет новый id) читал ссылку на уже удалённое
    # сообщение — лишний вызов delete в пустоту и, что хуже, ложное ощущение,
    # будто подсказка учтена.
    await state.update_data(name=name, prompt_id=None)
    ...
    # Responder допишет prompt_id в состояние сразу после отправки, а если
    # диалог к тому моменту закроется — уберёт подсказку из чата (см. _settle).
    ui.reply(
        message,
