...
export async function applyPayment(ownerId, id, amount) {
  ...
  await db
    ...
    .where(own(ownerId, id))
    .run();
  await recordOperation(s, {
    type: 'payment',
    amount,
    lessonsDelta: lessons,
    balanceAfter: s.balance + lessons,
    remainderAfter: remainder,
  });
  return { student: await getStudent(ownerId, id), lessons, remainder, prevRemainder: s.remainder };
}
...
async function shiftBalance(ownerId, id, type, delta) {
  ...
  return await getStudent(ownerId, id);
}
...
export async function undoLastOperation(ownerId) {
