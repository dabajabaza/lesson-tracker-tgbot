...
коммитом в конце обработки, внутри пер-пользовательского замка — поэтому двойной тап
не даёт гонки на балансе, а частично применённых операций не бывает.

Ответ пользователю уходит **до** коммита, и это осознанный разворот прежнего порядка:
если отправка сорвалась, транзакция откатывается целиком и повтор безопасен. Прежний
порядок («сначала коммит») в той же ситуации оставлял запись, пользователь видел ошибку
и вводил сумму снова — оплата задваивалась. Для денег дубль хуже потери.
